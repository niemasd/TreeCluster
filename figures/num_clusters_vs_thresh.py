...
hcv = {'t':[],'n':[]}
for l in [line.strip() for line in open(d+'LANL_web_2008_HCV_core_num_clusters_vs_thresh.tsv').read().strip().splitlines()]:
    t,n = l.split('\t'); hcv['t'].append(float(t)); hcv['n'].append(int(n))
hiv_lanl = {'t':[],'n':[]}
for l in [line.strip() for line in open(d+'LANL_web_2016_HIV1_pol_ALL_num_clusters_vs_thresh.tsv').read().strip().splitlines()]:
    t,n = l.split('\t'); hiv_lanl['t'].append(float(t)); hiv_lanl['n'].append(int(n))
hiv_sd = {'t':[],'n':[]}
for l in [line.strip() for line in open(d+'SD_SubtypeB_639_num_clusters_vs_thresh.tsv').read().strip().splitlines()]:
    t,n = l.split('\t'); hiv_sd['t'].append(float(t)); hiv_sd['n'].append(int(n))
flu = {'t':[],'n':[]}
for l in [line.strip() for line in open(d+'Influenza_TypeA_Human_SegmentPB2_FullLengthOnly_num_clusters_vs_thresh.tsv').read().strip().splitlines()]:
...
sns.plt.ylabel('Number of Clusters',fontsize=14)
sns.plt.title('Number of Clusters vs. Distance Threshold',fontsize=18)
sns.plt.show()
fig.savefig('num_clusters_vs_thresh.pdf', format='pdf', bbox_extra_artists=(legend,), bbox_inches='tight')
plt.close()